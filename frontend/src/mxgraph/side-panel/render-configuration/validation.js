...
export const checkDb2RequiredFields = state => {
    ...
    const { customSql, operation, readingInParallel } = state;
...
export const checkAWSRequiredFields = state => {
    ...
    const { anonymousAccess, format } = state;
...
export const validations = {
    ...
    [STORAGES.KAFKA.value]: state => {
        if (state.operation === READ || state?.isConnectionPage) {
            return isAnyEmpty('bootstrapServers', 'subscribe')(state);
        }
        return isAnyEmpty('bootstrapServers', 'topic')(state);
    },
